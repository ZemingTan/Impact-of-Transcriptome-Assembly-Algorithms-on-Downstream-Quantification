...
# 步骤1：加载数据
data = pd.read_csv('result_human/Cufflinks.csv')  # 替换为实际文件路径

# 步骤2：准备数据
X = data['Groundtruth'].values.reshape(-1, 1)  # 自变量（特征）
y = data['Cufflinks'].values                   # 因变量（目标）
...
print("异常点详细信息：")
print(outliers[['Transcript', 'Groundtruth', 'Cufflinks']])
outliers.to_csv("abnormal_cufflinks.csv", 
               columns=['Transcript', 'Groundtruth', 'Cufflinks', 'is_outlier'],
               index=False)
...
#                             & (outliers['Cufflinks'] < 5000) & (outliers['Cufflinks'] > 1000)] # 筛选异常点
# 标注异常点名称

#for idx, row in filtered_outliers.iterrows():
#    plt.text(row['Groundtruth'], row['Cufflinks'], 
#             row['Transcript'], 
...
plt.ylabel('Cufflinks(TPM)',fontsize=15)
plt.legend()
plt.savefig('Cufflinks.png',dpi=300, bbox_inches='tight')
plt.show()
...
groundtruth_percent = outliers['Cufflinks'].sum()/data['Cufflinks'].sum()
print("="*50)
print(f"{groundtruth_percent:.2f}")  # 保留两位小数
print("="*50)