...
# 步骤1：加载数据
data = pd.read_csv('result_human/Stringtie.csv')  # 替换为实际文件路径

# 步骤2：准备数据
X = data['Groundtruth'].values.reshape(-1, 1)  # 自变量（特征）
y = data['Stringtie'].values                   # 因变量（目标）
...
print("异常点详细信息：")
print(outliers[['Transcript', 'Groundtruth', 'Stringtie']])
outliers.to_csv("abnormal_Stringtie.csv", 
               columns=['Transcript', 'Groundtruth', 'Stringtie', 'is_outlier'],
               index=False)
...
#                             & (outliers['Stringtie'] < 5000) & (outliers['Stringtie'] > 1000)] # 筛选异常点
# 标注异常点名称

#for idx, row in filtered_outliers.iterrows():
#    plt.text(row['Groundtruth'], row['Stringtie'], 
#             row['Transcript'], 
...
plt.ylabel('Stringtie(TPM)',fontsize=15)
plt.legend()
plt.savefig('Stringtie.png',dpi=300, bbox_inches='tight')
plt.show()
...
groundtruth_percent = outliers['Stringtie'].sum()/data['Stringtie'].sum()
print("="*50)
print(f"{groundtruth_percent:.2f}")  # 保留两位小数
print("="*50)